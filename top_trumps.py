# Project option [1] Top Trumps

# Required task completed:
#   1. Generated a random number between 1 and 151 to use as the Pokémon ID number
#   2. Used the Pokémon API get a Pokémon based on its ID number
#   3. Created a dictionary that contains the returned Pokémon's name, id, height and weight
#   4. Got a random Pokémon for the player and another for their opponent
#   5. Asked the user which stat they want to use (id, height or weight)
#   6. Compared the player's and opponent's Pokémon on the chosen stat to decide who wins

# Extension tasks completed:
#   1. Asked the user for their username
#   2. Used a different stat for the Pokémon from the API (number of moves)
#   3. Got 3 random Pokémon and asked the player to select which one they want to use
#   4. Asked the player to select how many rounds they want to play
#   5. Played the number of rounds asked for; the player with the most rounds won, wins the game
#   6. Recorded the match and round results in a csv
#   7. Allowed the player and their opponent to take turns choosing the stat
#   8. Handled user input error with appropriate error messages
#   9. Printed user's player stats from the csv, at the end of the match
#       (total matches played / total matches won / match win rate / highest score)
#   10. Used newlines to make the printed output more user-friendly and more like reading a Top Trump card
